...
class iouEval:
    ...
    def addBatch(self, x, y):   #x=preds, y=targets
        ...
        if (x.size(1) == 1):
            ...
            # La funzione scatter_(dim,index,src) è utilizzata per aggiornare un tensore assegnando specifici valori in posizioni definite. Dove :
            # dim --> è la dimensione del vettore x per il quale si vogliono aggiornare i valore (in questo caso la seconda dimensione, ovvero quella del assegnazione della classe)
            # index --> è un tensore che fornisce gli indici in cui inserire i valori.
            # src --> sono i valori da inserire.
            # Quindi, utilizzando dim=1 in scatter_, stai dicendo: "Voglio aggiornare i valori lungo la dimensione delle classi". Gli indici forniti in index determinano quali classi (canali) in x_onehot devono essere aggiornati per ogni posizione spaziale dell'immagine (ogni pixel).
            # Dopo l'esecuzione di questa operazione, x_onehot diventa una rappresentazione one-hot delle previsioni del modello, dove per ogni posizione dell'immagine, solo il canale corrispondente alla classe predetta ha un valore di 1, mentre tutti gli altri canali hanno un valore di 0.
