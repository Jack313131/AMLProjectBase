...
class DownsamplerBlock (nn.Module):
    def __init__(self, ninput, noutput):
        ...
        self.deq = torch.ao.quantization.DeQuantStub()
    ...
    def forward(self, input):
        ...
        return self.deq(output)

    def quantize(self, confg):
        # self.qbnrelu = QuantizedBNReLU (self.bn)
        # self.qbnrelu.qconfig = torch.ao.quantization.get_default_qconfig(confg)
        # self.qbnrelu = torch.ao.quantization.fuse_modules(self.qbnrelu, [['bn', 'relu']])
        # self.qbnrelu = torch.ao.quantization.prepare(self.qbnrelu)
        # del self.relu
        # del self.bn
        return
    ...
    def freeze(self): ##salvo qi e qo
        # self.qbnrelu = torch.ao.quantization.convert(self.qbnrelu)
        return
...
class QuantizedBNReLU(nn.Module):
...
class non_bottleneck_1d(nn.Module):
    ...
    def quantize(self, confg):
        self.qblock = QuantizedBN1d(self.conv3x1_1,
...
class QuantizedBN1d(nn.Module):
    def __init__(self, conv3x1_1, conv1x3_1, bn1, conv3x1_2, conv1x3_2, bn2):
        super().__init__()
        self.quant = torch.ao.quantization.QuantStub()
        self.conv3x1_1 = conv3x1_1
        self.relu1 = nn.ReLU()
        self.conv1x3_1 = conv1x3_1
        self.bn1 = bn1
        self.relu2 = nn.ReLU()
        self.conv3x1_2 = conv3x1_2
        self.relu3 = nn.ReLU()
        self.conv1x3_2 = conv1x3_2
        self.bn2 = bn2
        self.deq = torch.ao.quantization.DeQuantStub()
...
class Encoder(nn.Module):
    ...
    def quantize(self, confg):
        self.initial_block.quantize(confg)
        for layer in self.layers:
            layer.quantize(confg)
...
class UpsamplerBlock (nn.Module):
    ...
    def quantize(self, config, num_bits=8):
...
class Net(nn.Module):
    ...
    def quantize(self, confg):
        self.encoder.quantize(confg)
        self.decoder.quantize(confg)
