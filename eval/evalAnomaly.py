...
def main():
    ...
    for path in glob.glob(os.path.expanduser(str(args.input[0]))):
        ...
        with torch.no_grad():
            ...
            if args.backgroundAnomaly:
                ...
                # Imposta a zero le probabilità per tutte le altre classi
    ...
    # ood_mask sarà True dove le etichette di ground truth sono 1 (anomalie) e False altrove.
    ood_mask = (ood_gts == 1)
    ...
    # ind_mask sarà True dove le etichette di ground truth sono 0 (non-anomalie) e False altrove.
    ind_mask = (ood_gts == 0)

    # ood_out contiene i punteggi di anomalia corrispondenti alle anomalie rilevate (dove ood_gts è 1).
    # ood_out contiene i punteggi di anomalia calcolati dal modello, ma solo per quei pixel che sono stati etichettati come anomalie nel ground truth (dove ood_gts è 1).
    # In altre parole, questa variabile raccoglie gli score di anomalia solo per le aree che sono state identificate come anomale nel ground truth.
    # # La dimensione di ood_out corrisponde al numero di pixel etichettati come anomalie dal modello.
    ood_out = anomaly_scores[ood_mask]
    # ind_out contiene i punteggi di anomalia per i dati non-anomali (dove ood_gts è 0).
    # ind_out, d'altra parte, contiene i punteggi di anomalia per i pixel che sono stati etichettati come non-anomali nel ground truth (dove ood_gts è 0).
    # Quindi, questa variabile raccoglie gli score di anomalia per le aree che sono considerate normali o non anomale nel ground truth.
    # La dimensione di ind_out corrisponde al numero di pixel etichettati come non-anomalie dal modello.
    ind_out = anomaly_scores[ind_mask]

    # ood_label è un array di 1, che rappresenta le etichette per i dati anomali nelle immagini di ground truth.
    # ind_label è un array di 0, che rappresenta le etichette per i dati non-anomali nelle immagini di ground truth.
    ...
    # Questa è una misura importante per valutare la performance di un modello di classificazione, in particolare in contesti dove c'è uno squilibrio tra le classi (come spesso accade nei problemi di rilevamento di anomalie).
    # La Precision-Recall Curve (PRC) mostra il trade-off tra la precisione (la percentuale di identificazioni positive che sono corrette) e il recall (la percentuale di positivi reali che sono stati identificati correttamente) a vari livelli di soglia. La curva PRC è particolarmente utile quando le classi sono sbilanciate.
    # L'AUC per la PRC (AuPRC) è l'area sotto la curva Precision-Recall. Un valore più alto di AUC indica una migliore capacità del modello di distinguere tra classi positive e negative. Un AuPRC di 1.0 rappresenta una classificazione perfetta, mentre un valore di 0.5 indica una performance non meglio di una classificazione casuale
    # un valore di AuPRC (Area Under the Precision-Recall Curve) vicino a 1 indica che il tuo modello è molto efficace nel distinguere le anomalie dalle non-anomalie. Significa che il modello ha una buona precisione e un buon recall per la classe di interesse (in questo caso, le anomalie). Un AuPRC elevato suggerisce che il modello non solo identifica correttamente un'alta percentuale delle anomalie reali (alto recall) ma anche che le etichette predette come anomalie sono effettivamente anomalie (alta precisione).
    # Questa è una metrica particolarmente importante in scenari dove le classi sono sbilanciate, come nel rilevamento di anomalie.
    ...
    # calcola il False Positive Rate (FPR) al 95% di True Positive Rate (TPR). Questa è una metrica usata per valutare quanto bene un modello riesce a identificare le anomalie.
    # True Positive Rate (TPR) rappresenta la proporzione di anomalie reali che il modello identifica correttamente.
    # False Positive Rate (FPR) rappresenta la proporzione di non-anomalie che vengono erroneamente identificate come anomalie dal modello.
    # Calcolare il FPR al 95% di TPR significa trovare il tasso di falsi positivi quando il modello ha un TPR del 95%. In altre parole, stai valutando quanto spesso il modello commette errori (falsi positivi) quando riesce a identificare correttamente il 95% delle anomalie reali.
    # Un FPR basso in questa situazione indica che il modello è efficace nel rilevare anomalie senza generare molti falsi allarmi.
    # Un FPR del 0% indica che non ci sono falsi positivi, ovvero che il modello non ha erroneamente identificato alcuna non-anomalia come anomalia. Al contrario, un FPR del 100% suggerisce che tutte le non-anomalie sono state erroneamente classificate come anomalie. In generale, un valore basso di FPR è desiderato, specialmente in un contesto come il rilevamento di anomalie, poiché indica che il modello ha una bassa frequenza di falsi allarmi.
